...
const toolsList: Tool[] = [
  ...
  },
  {
    ...
    description: "Capture screenshots of Binance futures chart for both 5m and 30m intervals in parallel",
    inputSchema: {
      type: "object",
      properties: {
        symbol: {
...
async function captureScreenshotOnPage(page: Page, symbol: string, interval: '5m' | '30m'): Promise<{ base64: string, symbol: string, interval: string }> {
...
async function captureScreenshot(symbol: string, interval: '5m' | '30m'): Promise<{ base64: string, symbol: string, interval: string }> {
  const page = await getTab();
  return captureScreenshotOnPage(page, symbol, interval);
}

async function captureScreenshotsAcrossTimeframes(symbol: string): Promise<{ '5m': { base64: string, symbol: string, interval: string }, '30m': { base64: string, symbol: string, interval: string } }> {
